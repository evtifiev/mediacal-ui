...
<template>
  <div v-loading="loading">
    <el-row>
        <div class="title">Список должностей</div>
        <!-- Operations -->
        <div class="operation">
            ...
            <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="refresh" v-has="[positions.request]"><i class="el-icon-refresh"></i> Обновить</el-dropdown-item>
                <el-dropdown-item command="add" v-has="[positions.add]"><i class="el-icon-plus"></i> Создать</el-dropdown-item>
            </el-dropdown-menu>
            </el-dropdown>
        </div>
         <!-- list -->
            ...
            <el-table-column width="80" align="center">
              <template slot-scope="scope">
                <el-tooltip content="Редактировать" placement="top">
                  <el-button type="text" icon="el-icon-edit" v-has="[positions.edit]" @click="handleEdit(scope.$index, scope.row)"></el-button>
                </el-tooltip>
                <el-tooltip content="Удалить" placement="top">
                  <el-button type="text" icon="el-icon-delete" v-has="[positions.remove]" @click="handleDel(scope.$index, scope.row)"></el-button>
                </el-tooltip>	
              </template>
            </el-table-column>
        </el-table>
        <!--Edit dialog-->
		<el-dialog title="Редактировать должность" :visible.sync="editFormVisible" :close-on-click-modal="false" width="30%">
    ...
    <!--Add dialog-->
		<el-dialog title="Добавить должность" :visible.sync="addFormVisible" :close-on-click-modal="false" width="30%">
  ...
  methods: {
      ...
      handleDel: function(index, row) {
        ...
        vm
            ...
            vm.listLoading = true;
            //NProgress.start();
            let position = { id: row.id };
            positions.remove.r(position).then(res => {
      ...
      editSubmit: function() {
        ...
        vm.$refs.editForm.validate(valid => {
            ...
            vm.$confirm("Подтвердить отправку?？", "Подсказка", {}).then(() => {
                vm.editLoading = true;
                //NProgress.start();
                let position = Object.assign({}, this.editForm);
                // para.birth = (!para.birth || para.birth == '') ? '' : util.formatDate.format(new Date(para.birth), 'yyyy-MM-dd');
                positions.edit.r(position).then(res => {
      ...
      addSubmit: function() {
        this.$refs.addForm.validate(valid => {
            ...
            this.$confirm("Подтвердить отправку？", "Подсказка", {}).then(() => {
                ...
                //NProgress.start();
                let position = Object.assign({}, this.addForm);
                // para.birth = (!para.birth || para.birth == '') ? '' : util.formatDate.format(new Date(para.birth), 'yyyy-MM-dd');
                positions.add.r(position).then(res => {
