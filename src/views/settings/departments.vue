...
<template>
  <div>
    <el-row>
        ...
        <div class="operation">
            ...
            <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="refresh" v-has="[departments.request]"><i class="el-icon-refresh"></i> Обновить</el-dropdown-item>
                <el-dropdown-item command="add" v-has="[departments.add]"><i class="el-icon-plus"></i> Создать</el-dropdown-item>
            </el-dropdown-menu>
            </el-dropdown>
        </div>
         <!-- list -->
            ...
            <el-table-column width="80" align="center">
              <template slot-scope="scope">
                <el-tooltip content="Редактировать" placement="top">
                  <el-button type="text" icon="el-icon-edit" v-has="[departments.edit]" @click="handleEdit(scope.$index, scope.row)"></el-button>
                </el-tooltip>
                <el-tooltip content="Удалить" placement="top">
                  <el-button type="text" icon="el-icon-delete" v-has="[departments.remove]" @click="handleDel(scope.$index, scope.row)"></el-button>
                </el-tooltip>	
              </template>
            </el-table-column>
        </el-table>
        <!--Edit dialog-->
		<el-dialog title="Редактировать подразделение" :visible.sync="editFormVisible" :close-on-click-modal="false" width="30%">
    ...
    <!--Add dialog-->
		<el-dialog title="Добавить подразделение" :visible.sync="addFormVisible" :close-on-click-modal="false" width="30%">
...
export default {
  ...
  methods: {
      ...
      fetchData: function() {
        let vm = this;
        vm.loading = true;
        departments.request.r().then(res => {
      ...
      handleDel: function(index, row) {
        ...
        vm
            ...
            vm.listLoading = true;
            //NProgress.start();
            let department = { id: row.id };
            departments.remove.r(department).then(res => {
      ...
      editSubmit: function() {
        ...
        vm.$refs.editForm.validate(valid => {
            ...
            vm.$confirm("Подтвердить отправку?？", "Подсказка", {}).then(() => {
                vm.editLoading = true;
                //NProgress.start();
                let department = Object.assign({}, this.editForm);
                // para.birth = (!para.birth || para.birth == '') ? '' : util.formatDate.format(new Date(para.birth), 'yyyy-MM-dd');
                departments.edit.r(department).then(res => {
      ...
      addSubmit: function() {
        this.$refs.addForm.validate(valid => {
            ...
            this.$confirm("Подтвердить отправку？", "Подсказка", {}).then(() => {
                ...
                //NProgress.start();
                let department = Object.assign({}, this.addForm);
                // para.birth = (!para.birth || para.birth == '') ? '' : util.formatDate.format(new Date(para.birth), 'yyyy-MM-dd');
                departments.add.r(department).then(res => {
