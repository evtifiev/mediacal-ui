  ...
  methods: {
    setInterceptor: function(resourcePermission) {
      myInterceptor = instance.interceptors.request.use(config => {
        ...
        if (!resourcePermission[config.method + ',' + perName]) {
          this.$message({
            message: 'Нет доступа, обратитесь к администратору системы.',
            type: 'warning'
          });
          return Promise.reject({
            message: 'Отсутствует разрешение'
          });
        }
        return config;
      });
    ...
    extendRoutes: function(routePermission) {
      ...
      if (!actualRouter || !actualRouter.length) {
        ...
        util.session('token','');
        // Interface hints
        return document.body.innerHTML = ('<h1>Доступ к учетной записи ограничен, обратитесь к системному администратору！</h1>');
      }
